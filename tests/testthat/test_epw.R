test_that("Epw class", {
    ...
    expect_error({epw$time_step <- 2}, "deprecated")
    ...
    expect_error(epw$holiday(TRUE), "Failed to change leap year indicator to TRUE")
    ...
    expect_error(epw$period(2), "Invalid data period index")
    expect_equal(epw$period(1, name = "test")$name, "test")
    expect_error(epw$period(1, name = "test"), "should not be the same as existing")
    expect_equal(epw$period(1, name = "Data")$name, "Data")
    expect_error(epw$period(1, start_day_of_week = "test"), "invalid day of week format")
    ...
    expect_error(epw$data(2), "Invalid data period")
    ...
    expect_error(epw$add(epw$data(start_year = 2016L), realyear = TRUE, warning = FALSE), "Missing or invalid date")
