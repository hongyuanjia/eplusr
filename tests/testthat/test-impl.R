...
test_that("Basic Table Implementation", {
    ...
    expect_equal(conditionMessage(catch_cnd(verbose_info(c("a", "b")))), "ab")
    expect_equal(conditionMessage(catch_cnd(verbose_info("a {b}"))), "a {b}")
    msg <- catch_cnd(verbose_info("a"))
    expect_s3_class(msg, "eplusr_message")
    expect_s3_class(msg, "eplusr_message_verbose")
    expect_s3_class(msg, "message")
    msg <- catch_cnd(verbose_info("a", class = "custom"))
    expect_s3_class(msg, "eplusr_message_custom")
    expect_silent(with_silent(verbose_info("a")))
