...
test_that("Transition v23.1 --> v23.2", {
    ...
    from <- "23.1"
    to <- "23.2"
    curve_name <- "UnitaryWAHP-AutogeneratedPLFCurve"

    expect_s3_class(
        class = "Idf",
        idfOri <- temp_idf(from,
            DistrictHeating = list("DH", "HW In", "HW Out"),
            Branch = list("Branch", ..3 = "DistrictHeating"),
            PlantEquipmentList = list("PlantEquip", ..2 = "DistrictHeating"),
            OtherEquipment = list("Other", "Steam"),
            `Meter:Custom` = list("CustomMeter", "DistrictHeating", "Key", "Some Var"),
            `Coil:Cooling:DX:SingleSpeed` = list("CDXS", ..27 = 10, ..28 = "SupplyTank"),
            `AirLoopHVAC:UnitarySystem` = list(
                "UnitaryWAHP",
                ..12 = "Coil:Heating:WaterToAirHeatPump:EquationFit",
                ..13 = "HWAHP",
                ..15 = "Coil:Cooling:WaterToAirHeatPump:EquationFit",
                ..16 = "CWAHP",
                ..42 = 3.0,
                ..43 = 80.0,
                ..44 = 0.0,
            ...
            `AirLoopHVAC:UnitarySystem` = list(
                "UnitaryDX",
                ..15 = "Coil:Cooling:DX:VariableSpeed",
                ..16 = "CDXVS",
                ..42 = 4.0,
                ..43 = 90.0,
                ..44 = 0.0,
            ...
            `Coil:Cooling:WaterToAirHeatPump:EquationFit` = list(
                ...
                "Autosize", "Autosize", "Autosize", "Autosize", 5.0,
                30, 27, 19, "TotCap", "SensCap", "Power"
            ),
            `Coil:Heating:WaterToAirHeatPump:EquationFit` = list(
                "HWAHP", "WaterIn", "WaterOut", "AirIn", "AirOut",
                "Autosize", "Autosize", "Autosize", 5.0,
                20, 20, 1.0, "HeatCap", "HeatPower"
            ),
            `Coil:Cooling:DX:VariableSpeed` = list(
                "CDXVS", "In", "Out", 2, 2, 10000, 1.0, 45, 0.2,
                "PLF", "CondIn", "AirCooled", 0, 0, 10, -25, NULL,
                NULL, 0, 2, "BasinSch",
                1000, 0.75, 3.0, 0.5, 1.2, 0.9, "C1-CapFT",
                "C1-CapFF", "C1-EIRFT", "C1-EIRFF",
                2000, 0.70, 3.1, 0.6, 1.3, 0.8, "C2-CapFT",
                "C2-CapFF", "C2-EIRFT", "C2-EIRFF"
            ),
            `Coil:Heating:DX:VariableSpeed` = list(
                "HDXVS", "In", "Out", 2, 2, 10000, 1.0, "HPLF",
                "DefEIR", -20, -5, 5, 0, 10, "ReverseCycle", "Timed",
                0.1, 500,
                1000, 3.0, 0.5, "H1-CapFT", "H1-CapFF", "H1-EIRFT",
                "H1-EIRFF",
                2000, 3.1, 0.6, "H2-CapFT", "H2-CapFF", "H2-EIRFT",
                "H2-EIRFF"
            ),
            .all = FALSE
        )
    ...
    expect_s3_class(idfTR <- transition(idfOri, to), "Idf")
    expect_equal(idfTR$version(), numeric_version("23.2.0"))

    expect_true(idfTR$is_valid_class("DistrictHeating:Water"))
    expect_equal(idfTR$`DistrictHeating:Water`$DH$value(1:3), list(
        Name = "DH",
        `Hot Water Inlet Node Name` = "HW In",
        `Hot Water Outlet Node Name` = "HW Out"
    ))
    expect_equal(idfTR$Branch$Branch$value(3)[[1L]], "DistrictHeating:Water")
    expect_equal(idfTR$PlantEquipmentList$PlantEquip$value(2)[[1L]], "DistrictHeating:Water")
    expect_equal(idfTR$OtherEquipment$Other$value(2)[[1L]], "DistrictHeatingSteam")
    expect_equal(idfTR$`Meter:Custom`$CustomMeter$value(2)[[1L]], "DistrictHeatingWater")

    expect_true(is.na(idfTR$`Coil:Cooling:DX:SingleSpeed`$CDXS$value(27)[[1L]]))
    expect_equal(idfTR$`Coil:Cooling:DX:SingleSpeed`$CDXS$value(28)[[1L]], 10)
    ...
    expect_equal(
        unname(unlist(idfTR$`Coil:Cooling:WaterToAirHeatPump:EquationFit`$CWAHP$value(20:22))),
        c(3, 80, 20)
    )
    expect_equal(idfTR$`Coil:Heating:WaterToAirHeatPump:EquationFit`$HWAHP$value(15)[[1L]], curve_name)

    expect_equal(unname(unlist(idfTR$`Coil:Cooling:DX:VariableSpeed`$CDXVS$value(10:12))), c(4, 90, 25))
    expect_equal(
        unname(unlist(idfTR$`Coil:Cooling:DX:VariableSpeed`$CDXVS$value(c(36:37, 48:49)))),
        c("C1-EIRFT", "C1-EIRFF", "C2-EIRFT", "C2-EIRFF")
    )
    expect_equal(
        unname(unlist(idfTR$`Coil:Heating:DX:VariableSpeed`$HDXVS$value(c(27:28, 36:37)))),
