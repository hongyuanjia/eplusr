...
describe("parse_idd()", {
    ...
    it("can parse IDD from string", {
        ...
        expect_equal(names(idd_parsed), c("version", "build", "data",
                                          "reference", "external"))
    })
    ...
    it("can parse class data", {
        ...
        expect_equal(idd_parsed$data$reference_class_name, list(NA_character_, "RefTestSlash"))
        ...
        expect_equal(idd_parsed$data$data_class[[1]],
                     data.table::data.table(
                         ...
                         unique_object = FALSE,
                         extensible = 0L,
                         reference_class_name = list(NA_character_)
                    )
        )
        expect_equal(idd_parsed$data$data_class[[2]],
                     data.table::data.table(
                         group_order = 2L,
                         group = "TestGroup2",
                         class_order = 2L,
                         ...
                         unique_object = TRUE,
                         extensible = 4L,
                         reference_class_name = list("RefTestSlash")
                    )
        )
        # }}}
    })
    ...
    it("can parse field data", {
        # {{{
        expect_equal(idd_parsed$data$data_field[[1]],
                     data.table::setnames(data.table::data.table(
                         ...
                         external_list = list(NA_character_),
                         unitsbasedonfield = FALSE,
                         `_field_name` = "Test Field",
                         `_unit` = "",
                         `_ip_unit` = "",
                         `_field` = "Test Field",
                         `_field_ip` = "Test Field"), "choice", "key"
                     )
        )

        expect_equal(idd_parsed$data$data_field[[2]],
                     data.table::setnames(data.table::data.table(
...
describe("$object()", {
    ...
    it("can stop when multiple class names are given", {
        expect_error(idd$object(c("TestSimple", "TestSlash")),
                     "class is not a string (a length one character vector).")
    })
})
...
describe("$external()", {
    it("can return all external data", {
        expect_equal(idd$reference(), idd_parsed$external)
    })
})
