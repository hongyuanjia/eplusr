...
library(eplusr)
eplus_ver <- get("LATEST_EPLUS_VER", asNamespace("eplusr"))
if (!is_avail_eplus(eplus_ver)) install_eplus(eplus_ver)
```
...
download IDD file for the latest EnergyPlus release, parse it and create an
`Idd` object that will be used whenever parsing all EnergyPlus models of that
version.

```{r idd_dl, eval = FALSE}
path_idd <- download_idd("latest", dir = tempdir())
use_idd(path_idd)

# OR
use_idd("latest", download = TRUE)
```
...
Now let's read an IDF file distributed with the latest EnergyPlus release. As we
have already got the IDD, we can just ignore the `idd` argument.
...
```{r read, out.lines = 10}
path <- path_eplus_example(eplus_ver, "5Zone_Transformer.idf")
model <- read_idf(path)
...
path_ddy <- path_eplus_weather(eplus_ver, "USA_CA_San.Francisco.Intl.AP.724940_TMY3.ddy")
