...
if (!eplusr::is_avail_eplus(8.8)) {
    if (eplusr:::is_windows()) {
        ...
        url <- paste0(tools::file_path_sans_ext(eplusr:::eplus_download_url(8.8)), ".tar.gz")
        dest <- file.path(tempdir(), basename(url))
        dl <- eplusr:::download_file(url, dest)
        eplus_dir <- file.path(tools::file_path_sans_ext(tools::file_path_sans_ext(basename(dest))), "EnergyPlus-8-8-0")
