...
that will be used whenever parsing all EnergyPlus models of that version.
...
| Object Info         | `$object_id()`           | Get object unqiue ID                              |
...
values of all fields. The type of each value will be consistent with the field
definition.
...
to their appearance sequences in the IDF file. You can find all valid `ID`s using
`$object_id()`.
...
`Idf` class provides 8 methods for modifying objects, including `$dup()`,
`$add()`, `$set()`, `$del()`, `$rename()`, `$insert()`, `$load()` and
`$paste()`.

Object IDs will be appended after new objects are added, and the most-newly
added object will alway have the max ID. Object IDs will never be reused, even
...
`$del()` will delete current objects specified by object IDs or names.
...
We got an error, becase directly deleting `MAT-CLNG-1` will introduce invaid
...
`$paste()` will also work only on that platfrom.

## Validate

`$validate()` checks if there are errors in current `Idf` under specified
validation level. You can customize what kind of errors to check by changin the
...
what valiation components those levels contain, use `level_checks()`.
...
The `final` validation level turns all components on. We can just trigger
invalid reference checking using `custom_validate()` function.
...
cases, we don't know what that infomation in advance, we can extract
invalid objects for different types directly using `$validate()`. Below, we
extract all objects that have invalid reference errors.
...
Just for demonstration, let's get the transformer input electric power at 11 p.m
...
job$report_data("transformer 1", "transformer input electric power", case = "example",
