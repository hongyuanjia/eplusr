...
```{r all_field}
def_mat <- model$definition(class = "Material")
def_mat
```
...
`$objects()` returns a list of `IdfObject`s. The names of returned list are
...
You can duplicate, add, insert, modify and delete objects using `$dup()`,
`$add()`, `$insert()`, `$set()` and `$del()` in `Idf`,
respectively.

Object IDs will be appended after `$dup()`, `$add()` and
`$insert()`, and the newly added object will have the max
ID. Object IDs will never be reused, even though their binded objects have
been deleted using `$del()`.
...
`$dup()` will duplicate objects specified by object IDs or names. If the
...
```{r dup}
model$dup(c("ROOF-1", "ROOF-1", "WALL-1"))
```

### Add new objects

You can add new objects using `$add()`. With `.default` being `TRUE`, all
...
Field values can be given in a list following patterns below:
...
* give both field names *without units* and values in pair, e.g. `` Name =
  ...
  `$definition("class_name")$field_name()`. Field names can also be given
...
`Idf` and `IdfObject`. Basic rules above of field values
provided also apply to `$set()`. For example:
...
```{r ref_by}
clng <- model$Material_NoMass$`MAT-CLNG-1`
clng$ref_by_object()
```
...
`.referenced` to `TRUE` and `.force` to `TRUE` to force deleting.

```{r del_force}
model$del("mat-clng-1", .referenced = TRUE, .force = TRUE)
```
...
You can get all weather data using `$data()`.

```{r epw_data}
epw_data <- epw_sf$data()
str(epw_data)
```
...
job <- model$run(epw_sf, wait = TRUE)
job
```
