...
All newly added (or duplicated), modified and deleted fields will be marked with
`(+)`, `(~)` and `(-)` respectively. The valid IDs will be appended after
`$add` and `$dup`, and the newly added (or duplicated) object will have the max
ID.
...
a special mark to prevent them accessible. This is done by purpose, in order to
