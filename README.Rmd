...
location (`C:/EnergyPlusVX-X-0` on Windows, `/usr/local/EnergyPlus-X-Y-0` on
...
input IDF file. The IDD file will be parsed and an `Idd` object will be created
and cached. That `Idd` object will be reused whenever parsing IDF files with
same version. For more details, please see `?use_idd` and `?idd`.
...
Shortcuts are provided (`"$"` and `"["`) to get objects in a single class. Class
names can be given in a style that `:` is replaced by underscore `_`. This is
handy because you can just use `model$Output_Variable` instead of
