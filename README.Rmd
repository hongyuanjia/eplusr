...
* Save the changed models into standard formats in the same way as IDFEditor
  distrubuted along with EnergyPlus.
...
All reading process starts with creating an `R6Class` called `eplus_model`. The
...
EnergyPlus v8.3 to v8.8 have been pre-parsed and stored. So you can just ignore
the `idd` argument if you are using those verions. If not, just pass the path
...
model in an text editor, except that each object binded an unique `ID` and only
first two lines of each object will be shown.
...
along with `$add` you will see below. All required fields are marked with `*`.
For example, you can find all valid fields in class `Material`:
...
All newly added (duplicated), modified and deleted fields will be marked with
`(+)`, `(~)` and `(-)` respectively. The valid IDs will be appended after
`$add` and `$dup`, and the newly added (duplicated) object will have the max
ID.
...
* give both field names and values in pair, e.g. `Name = "Test", "Sepcific
...
Note that only minimum fiels will be added by default. But you can change it by
...
Note that the IDs of deleted objects are invalid after `$del` and cannot be
applied to methods `$set`, `$dup` and `$del`, of course. However, unless you
save the model, the deleted objects are still there internally but with
a special mark to prevent them accessable. This is done by purpose, in order to
provide a new method call `$undo` in the future, which will enable you to
un-delete the objects.
...
any value range exceedings, invalid autosizable and autocalculatable fields.
